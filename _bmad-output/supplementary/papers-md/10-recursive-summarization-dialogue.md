# 10-recursive-summarization-dialogue

**Source:** `10-recursive-summarization-dialogue.pdf`

---

## Recursively Summarizing Enables Long-Term Dialogue Memory in Large Language Models

Qingyue Wang [a], Yanhe Fu [b], Yanan Cao [b,] _[∗]_, Shuai Wang [a], Zhiliang Tian [c], Liang
Ding [d]


_a_ _the Department of Computer Science and Engineering, Hong Kong University of Science and_
_Technology, Hong Kong, China_
_b_ _the institute of information engineering, Chinese Academy of Sciences, Beijing, China_
_c_ _the College of Computer, National University of Defense Technology, Changsha, China_
_d_ _the University of Sydney, Sydney, Australia_
...
_Email addresses:_ qingyue.wang@ust.hk (Qingyue Wang), fuyanhe@iie.ac.cn
...
_Preprint submitted to Neurocomputing_ _August 26, 2025_
...
to enable the LLM to model the extremely long dialog context. We release our


[code in https://github.com/qingyue2014/Rsum.](https://github.com/qingyue2014/Rsum)


_Keywords:_ recursive summary, long-term memory, large language models,
...
Recently, large language models (LLMs), such as ChatGPT [1] and GPT-4 (Achiam
...
Large context windows allow many LLMs [2] to process entire dialog histories,
...
2 Recent GPT-4 supports 128,000 tokens and LLama3(MetaAI, 2024) supports 1,040,000 to
...
**[Dialogue Context] [About 20 turns]**


Do you like music?


Yes, you could say it is a great
source of joy for me.


Do you have a favorite genre? I
enjoy country music.


Everything really, I compose as
...
Do you have another job or do you
compose music for a living?


**[Response]**


As an AI language model, I don't have a
job in the traditional sense.


I don't have another job, composing
...
Figure 1: **A long-term conversation example** from the Multi-Session Chat Dataset (Xu et al.,
...
_2.2. Long-term Open-Domain Dialogue_
...
(a) Retriever method (b) MemoChat (c) MemoryBank (d) Ours
...
is also called _Multi-Session Dialogue_ . The goal of the task is to generate context

relevant and highly consistent responses to the user based on past sessions and


current context. Formally, each dialogue can be written as _D_ = _{S, C_ _t_ _, r_ _t_ _}_ . Here,


_S_ = _{S_ 1 _, S_ 2 _, ..., S_ _N_ _}_ represents _N_ past sessions and each of the sessions consists


of multiple utterances between two speakers. _r_ _t_ is the ground truth response to _C_ _t_


with background sessions _S_ . _C_ _t_ = _{u_ 1 _, r_ 1 _, ..., u_ _t_ _}_ denotes the dialogue context of


the current session at _t_ step, where _u_ and _r_ represent the utterances from the user
...
|I actually really enjoying walking and running<br>I like going to the gym to work out.<br>……|1|
|---|---|
|I actually really enjoying walking and running<br>_……_<br>I like going to the gym to work out.||
...
|…… I like going to the gym to work out.|Col2|
|---|---|
|Any luck with the search for a new gym?<br>Yes, I am joining the one near me. It is open<br>24 hours so I can go whenever I want.<br>_……_|2|
|Any luck with the search for a new gym?<br>Yes, I am joining the one near me. It is open<br>24 hours so I can go whenever I want.<br>_……_||
...
Figure 3: **The schematic overview of our method** . The model uses the first session to generate
...
_P_ ( _r_ _t_ _|C_ _t_ _, S_ ) = _P_ ( _r_ _t_ _|C_ _t_ _, M_ _N_ ) _P_ ( _M_ _N_ _|S_ ) _,_ (1)


where _M_ _i_ represents the available memory when the _i_ -th session is finished. And


_P_ ( _M_ _N_ _|S_ ) = [�] _[N]_ _i_ =1 _[P]_ [(] _[M]_ _[i]_ _[|][S]_ _[i]_ _[, M]_ _[i][−]_ [1] [)][ is a sequential or Markov process where each]


memory _M_ _i_ of session _i_ depends only on the current session and the previous
...
finish two tasks, i.e., **memory iteration** and **memory-based response genera-**
...
by carrying multiple “hard operations” on summaries, such as _replace_, _append_,


and _delete_, which rely on high-quality dialogue with operation labels. However,
...
_M_ _i_ = _**LLM**_ ( _S_ _i_ _, M_ _i−_ 1 _,_ P _m_ ) _,_ (2)


where _M_ _i_ = _{m_ 1 _, m_ 2 _, ..., m_ _J_ _}_ denotes multiple sentences, containing summa

rized key information from the session _S_ _i_, and P m is the prompt of LLM for


generating new memory. The memory iteration will be repeated _N_ times until


all previous sessions end, where we can obtain the latest memory _M_ _N_ . Take the
...
_Prompt Construction._ To enable LLM to efficiently carry the memory iteration
...
instructions helps the LLM better understand and execute the memory iteration [3] .


3 [https://platform.openai.com/docs/guides/prompt-engineering](https://platform.openai.com/docs/guides/prompt-engineering)
...
_4.2. Memory-based Response Generation_
...
_r_ _t_ = _**LLM**_ ( _C_ _t_ _, M_ _N_ _,_ P r ) _,_ (3)


where _P_ _r_ is a prompt of the generator. Especially, taking the generated memory


_M_ _N_ and current session _C_ _t_, we ask the LLM again to generate a response. In
...
_Prompt Construction._ The the prompt for memory-based response generation is
...
**Input:** A long-term dialog _D_ = _{S, C_ _t_ _}_ consisting multiple sessions with
...
**3** _M_ _i_ = _LLM_ ( _S_ _i_ _, M_ _i−_ 1 _, P_ _m_ );


// Update memory when a session ends


**4** _r_ _t_ = _LLM_ ( _C_ _t_ _, M_ _N_ _, P_ _r_ );
...
_MSC dataset._ is the largest human-human long conversations dataset so far. The
...
_Carecall._ is a Korean open-domain multi-session dataset, which is used for mon

itoring patient health. For a fair comparison, we use the public machine-translated


English version [4] in the experiments.
...
4 [https://github.com/naver-ai/carecall-memory](https://github.com/naver-ai/carecall-memory)
...
_Automatic Metrics._ We employ BLEU-1/2 (Papineni et al., 2002), F1 with refer
...
_Human Evaluation._ Many works point out that automatic evaluation metrics are
...
ria are discussed as follows: (1) _Engagingness_ : It evaluates whether the chatbot
...
ally appropriate, encouraging users to keep chatting. (2) _Coherence_ : It measures
...
stays relevant, enhancing user engagement. (3) _Consistency_ : It assesses whether
...
_LLM Evaluation._ Recently, LLM-as-a-Judge strategy (Pan et al., 2023) has been


widely used in evaluating generation tasks. Some works reveal minimal deviation


of GPT-4’s evaluation from humans ( _>_ 0.85 agreements) in dialog quality (Zhang


et al., 2023a). Inspired by this, we employ the GPT-4 as an advanced evaluator,


using two common methods to assess the quality of generated responses. (1) _Sin-_


_gle model evaluation_ (Lu et al., 2023a): we prompt GPT-4 to rate the responses


individually from the three aspects, i.e., engagingness, coherence and consistency


with an integer scale from 1 (very bad) to 100 (very good). (2) _Pairwise model_


_evaluation_ (Dubois et al., 2024): we ask the GPT-4 to directly compare two anony
...
_Context-only Approach._ It is the most naive approach to directly employ the LLM
...
_Retrieval-based Approach._ Many previous works (Xu et al., 2022a) employ re

trievers to filter key information and then include top- _k_ documents into inputs to


assist long-context dialogs. For the long-term dialog, the top- _k_ documents refer
...
**ChatGPT-BM25** and **ChatGPT-DPR**, respectively.


_Memory-based Approach._ Recent works employ a summarizer to abstract im
...
MemoryBank (Zhong et al., 2024) and MemoChat (Lu et al., 2023a). _Memory-_


_Bank_ proposes a human-like long-term memory mechanism, which creates or
...
baseline, named **ChatGPT-MemoryBank** . Differently, _MemoChat_ maintains the
...
5 [https://github.com/THUDM/ChatGLM2-6B](https://github.com/THUDM/ChatGLM2-6B)
6 For convenience’s sake, the following “ChatGPT” refer to the gpt-3.5-turbo-0301 version.
...
_Backbone LLMs._ We employ OpenAI ChatGPT “ _gpt-3.5-turbo-0301_ ”, “ _Llama2-_


_7B_ ” and “ _ChatGLM2-6B_ ” in the main experiments, “ _text-davinci-003_ ” and “ _Llama2-_


_7B_ ” (Touvron et al., 2023) in the analysis to show the universality, “ _longlora-_


_8k_ ” (Chen et al., 2024b) and ChatGPT-16k “ _gpt-3.5-turbo-16k_ ” as the backbones
...
_Retrievers._ Considering the scale of past utterances is not large enough to use the
...
_Memory-based Approaches._ The implementation and prompt design of the Mem
...
_LLM evaluations._ We use the GPT-4 model (version _“gpt-4-0314”_ ) as the eval
...
_Automatic Metrics Results._ In Table 3, we compare different methods over ses
...
_Human Evaluation Results._ We also present the results of human evaluations on
...
_6.2. LLM Evaluation_


_Single Model Evaluation._ Table 4 reports the GPT-4’s evaluation metrics results
...
overall quality of generated responses, i.e., ChatGPT-Rsum _>_ ChatGPT-MemoryBank


_>_ ChatGPT-MemoChat _>_ ChatGPT. Given this, we mainly present the LLM eval
...
_Pairwise Models Evaluation._ Furthermore, we randomly sample 1000 generated
...
|Ours win tie Ours loss<br>96.2 2.21.6<br>-<br>61.3 33.0 5.7<br>t<br>-<br>48.2 39.9 11.9|Col2|Ours win tie Ours loss|Col4|
|---|---|---|---|
|~~-~~<br>t<br>~~-~~<br><br>96.2<br>2.21.6<br>61.3<br>33.0<br>5.7<br>48.2<br>39.9<br>11.9<br>Ours win<br>tie<br>Ours loss|96.2<br>2.21.6<br>61.3<br>33.0<br>5.7<br>48.2<br>39.9<br>11.9|96.2<br>2.21.6<br>61.3<br>33.0<br>5.7<br>48.2<br>39.9<br>11.9|96.2<br>2.21.6<br>61.3<br>33.0<br>5.7<br>48.2<br>39.9<br>11.9|
|t<br>~~-~~<br>|t<br>~~-~~<br>|t<br>~~-~~<br>|t<br>~~-~~<br>|
|||||



Figure 4: **Comparative win rate of our method and competitive baselines,** including ChatGPT,


ChatGPT-MemoChat, and ChatGPT-MemoryBank.


_6.3. Ablation Study_
...
tions ( **RQs** ): _**RQ1**_ _: What is the quality of the generated memory?_ _**RQ2**_ _: What_


_errors may occur in memory generation?_ _**RQ3**_ _: Is the proposed method robust to_


_other LLMs?_ _**RQ4**_ _: Can our zero-shot method be effectively applied in few-shot_
...
5 in Table 3 (Ours _>_ ChatGPT-MemoryBank), we suppose that the accuracy of
...
|Sess|ion 4|
|---|---|
|ChatGP~~T-~~MemoryBank<br>ChatGP~~T-~~Rsum(Ours)|ChatGP~~T-~~MemoryBank<br>ChatGP~~T-~~Rsum(Ours)|
...
driving so I see animals.


User: I ended up giving in and getting my


daughters the cat.....Well when you have daughters


you sort of give in to them. She named it Angie.


User: I just saw the best movie on netflix.


Bot: What movie did you see?


User: It’s a documentary called The Social Dilemma.
...
The user’s daughters now have a
...
User: Enjoys watching TV, reading,
...
My daughters named the cat.


User: I think the documentary
...
generated memory. 1) _Fabricated facts_ refers to the memory containing some in

formation that the dialog history cannot verify. In the first case, the bot walks to


work not to see animals. 2) _Incorrect relationships_ refers that the generated mem

ory concludes error causal or references from history. In the second dialogue, the


user gave in to her daughter, rather than that cat. 3) _Missing details_ refers that the
...
|Col1|Ch<br>Ch|Col3|Col4|Col5|
|---|---|---|---|---|
||Ch<br>Ch<br>|Ch<br>Ch<br>|atGPT<br>atGP~~T-~~MemoChat<br>|atGPT<br>atGP~~T-~~MemoChat<br>|
||~~Ch~~<br>Ch|~~Ch~~<br>Ch|~~tGPT-MemoryBa~~<br>atGP~~T-~~Rsum (Our|~~k~~<br>s)|
||~~Ch~~<br>Ch|~~Ch~~<br>Ch|||
||||||
||||||


Figure 7: **The average number of tokens** in


generated responses.


|Vanilla LLM<br>LLM-Rsum(Ours)|Col2|
|---|---|
|||



language models, i.e., “text-davinci-003” _>_ “Llama2-7b”, indeed understand the
...
I'm spend way too much money on shoes


I like shoes, too. Especially cute sandals.


_……_


I buy basketball shoes that I don't even wear.
...
|Col1|Memory|Col3|
|---|---|---|
|**Gt. Memory:**Bot: I spend a lot of money on shoes. I like to buy<br>basketball shoes. I don‘t wear all the shoes I have.<br>**MemoryBank:**The assistant spends too much money on shoes.<br>Ours: Bot:Likes to buy basketball shoes but doesn't wear them often. <br>|||


|Col1|Retrieved Utterances|Col3|
|---|---|---|
|User: Well, I do need to make money somehow. How else can I feed my<br>love of shopping?<br>|||


|How does your shopping addiction go? Current Session|User: Well, I love of shop|
|---|---|
|||
||Responses|
...
the user mentions _“shopping addiction”_ at the current turn, referring to the bot’s
...
_6.6. Complementary to Existing Works_
...
_Retrieval-based LLMs.._ The effectiveness of retrieval-based methods on the MSC
...
_Long-context LLMs.._ To process the entire context and reduce the information
...
0613”) and GPT-4o [8] to verify the effectiveness of our proposed framework. To en
...
7 [https://huggingface.co/Yukang/Llama-2-7b-longlora-8k](https://huggingface.co/Yukang/Llama-2-7b-longlora-8k)
8 [https://openai.com/index/hello-gpt-4o/](https://openai.com/index/hello-gpt-4o/)
...
_Limitations._ One of the primary limitations of our method is that it does not ac
...
_Future work._ One promising direction for future work is to explore the effec
...
session ID _i_ indicates there are _i −_ 1 history conversation sessions that happened
...
You are an advanced AI designed for engaging in natural, personality-based conversations.
...
You are an impartial judge. You will be shown a Conversation Context, Personality of
...
Hi! We are a group of researchers working on Artificial Intelligence. In this task, we will ask
...
[MetaAI, 2024. Llama 3. URL: https://llama.meta.com/llama3/. ac-](https://llama.meta.com/llama3/)
...
[tion for Computational Linguistics. URL: https://doi.org/10.3115/](https://doi.org/10.3115/1073083.1073135)


[1073083.1073135.](https://doi.org/10.3115/1073083.1073135)
...
Research, F.A., 2019. Faiss. [URL: https://ai.meta.com/tools/](https://ai.meta.com/tools/faiss/)


[faiss/. accessed: 2024-08-26.](https://ai.meta.com/tools/faiss/)
...
for Computational Linguistics (Volume 1: Long Papers), pp. 1714–1729.
